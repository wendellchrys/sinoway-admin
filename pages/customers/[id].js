...
const Default = ({ dataCityOption = [], dataCity = [], getData = [] }) => {
   ...
   const prefixSelector = (
      <Form.Item name="prefix" noStyle>
         <Select onChange={changePrefix} style={{ width: 70 }}>
            <Select.Option value="55" defaultValue>
          +55
            </Select.Option>
         </Select>
      </Form.Item>
   ...
   function updatePassword(pass) {
      api
         .post("/staff/updatePasswordCustomer", {
            _id: id,
            password: pass,
         })
         .then((res) => {
...
Default.getInitialProps = async ({ req, query }) => {
   if (!req?.headers?.cookie) {
      ...
      const getData = await api.get("/customers/" + query.id, {
         headers: req ? { cookie: req.headers.cookie } : undefined,
      });

      getData.data["password"] = "";

      const getDataCity = await api.get("/turkey", {
         headers: req ? { cookie: req.headers.cookie } : undefined,
      });
      const dataManipulate = [];
